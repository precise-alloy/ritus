...
| GitHub | `#` prefix (requires `GITHUB_REPO_URL` in `.ritus/.env.local`) | `'#18'` (quotes required - `#` is a shell comment character) |

Auto-detection logic:

1. Filters to providers with valid credentials (`.ritus/.env.local` keys present).
...
bun run "<plugin-root>/scripts/remote-api.ts" check-env                              # verify .ritus/.env.local + required keys
...
bun run "<plugin-root>/scripts/remote-api.ts" github comments <PR_URL_OR_#NUMBER> [count|unresolved]  # PR review comments; `unresolved` = only comments in unresolved review threads
...
Bare numeric IDs (e.g. `12345`) are also accepted when `AZURE_DEVOPS_ORG` and `AZURE_DEVOPS_PROJECT` are set in `.ritus/.env.local`.

### GitHub PR URLs

```text
https://github.com/<owner>/<repo>/pull/<number>
#<number>                                         # requires GITHUB_REPO_URL in .ritus/.env.local
```
...
Ensure the user has a `.ritus/.env.local` file with the keys for the providers they use:
...
To generate a tailored `.ritus/.env.example` from the project's `team.yml` provider config:
...
in their `env:` block - users pick their own env var names. Routing config lives in `team.yml`; `.ritus/.env.local` carries
only credentials.
...
```dotenv
# .ritus/.env.local - credentials with natural names
