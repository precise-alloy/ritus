...
- **Read-only, with three exceptions:** you may write/update the **review document**, append to the
  `docs/tasks/{branch-slug}/exploration.md` **exploration log**, and append `DECISION-NNN` entries to
  `docs/DECISIONS.md`. Touch nothing else — no source files, no task files (those come after human approval).
...
If the ticket or requirement includes image attachments (screenshots, mockups, annotated images), download them first:
